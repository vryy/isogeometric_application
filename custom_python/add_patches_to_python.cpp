...
void IsogeometricApplication_AddPatchesToPython_Helper(const std::string& Prefix)
{
    ...
    .def("HasGridFunction", &PatchType::template HasGridFunction<Variable<TDataType> >)
    .def("HasGridFunction", &PatchType::template HasGridFunction<Variable<array_1d<TDataType, 3> > >)
    .def("HasGridFunction", &PatchType::template HasGridFunction<Variable<VectorType> >)
