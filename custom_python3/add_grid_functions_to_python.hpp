...
template<class TGridFrunctionType>
typename TGridFrunctionType::DataType GridFunction_GetValue1(TGridFrunctionType& rDummy, const pybind11::list& xi)
{
...
template<class TGridFrunctionType>
pybind11::list GridFunction_GetDerivative1(TGridFrunctionType& rDummy, const pybind11::list& xi)
{
    std::vector<double> xi_vec;
    for (auto v : xi)
        xi_vec.push_back(v.cast<double>());
...
void IsogeometricApplication_AddGridFunctionsToPython(pybind11::module& m)
{
    ...
    class_<ControlPointGridFunctionType, typename ControlPointGridFunctionType::Pointer>
    (m, ss.str().c_str())
    .def(init<typename FESpace<TDim>::Pointer, typename ControlGrid<ControlPoint<double> >::Pointer>())
    ...
    class_<DoubleGridFunctionType, typename DoubleGridFunctionType::Pointer>
    (m, ss.str().c_str())
    .def(init<typename FESpace<TDim>::Pointer, typename ControlGrid<double>::Pointer>())
    ...
    class_<Array1DGridFunctionType, typename Array1DGridFunctionType::Pointer>
    (m, ss.str().c_str())
    .def(init<typename FESpace<TDim>::Pointer, typename ControlGrid<array_1d<double, 3> >::Pointer>())
    ...
    class_<VectorGridFunctionType, typename VectorGridFunctionType::Pointer>
    (m, ss.str().c_str())
    .def(init<typename FESpace<TDim>::Pointer, typename ControlGrid<Vector>::Pointer>())
