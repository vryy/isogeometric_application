...
{
public:
    ISOGEOMETRIC_CLASS_POINTER_DEFINITION(SpatialPoint);
...
{
    public:
        ISOGEOMETRIC_CLASS_POINTER_DEFINITION(SpatialKey);
...
public:
    ...
    /// Pointer definition
    ISOGEOMETRIC_CLASS_POINTER_DEFINITION(AutoCollapseSpatialBinning);
    ...
    //                std::cout << "node exist in cell, return its id" << std::endl;
            ...
            mPointList.push_back(Kratos::shared_ptr<SpatialPoint>(new SpatialPoint(++mLastNode, X, Y, Z)));
...
private:
    ...
    std::vector<Kratos::shared_ptr<SpatialPoint> > mPointList;
