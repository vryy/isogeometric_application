...
//        KRATOS_WATCH(M)
    ...
    template<class ValuesContainerType, class ValuesContainerType1, class ValuesContainerType2>
            ...
            ValuesContainerType2& ik) // new knot vector
    {
        ...
        while (b < m)
        {
            ...
            for (i = lbz; i <= ph; i++)
            {
                mpi = std::min(d, i);
                j = std::max(0, i - t);
                assert(j <= mpi); // this is crucial to have the loop correct, otherwise we have to set
                        // ebpts[i] manually to zero
                #ifdef SIZE_CHECK
                if (j > mpi)
                    KRATOS_ERROR << "Invalid condition found";
                #endif
    ...
    {
        typedef typename ValuesContainerType::DataType DataType;
        ...
        for (int i = 0; i < n1; ++i)
        {
            std::vector<DataType> ctrl_v(n2);
            std::vector<DataType> ictrl_v(n2); // n2 is just temporary size
            ...
            ComputeBsplinesDegreeElevation1D(d2, ctrl_v, k2, t2, ictrl_v, ik2);
        ...
        for (int j = 0; j < new_n2; ++j)
        {
            std::vector<DataType> ctrl_u(n1);
            std::vector<DataType> ictrl_u(n1); // n1 is just temporary size
            ...
            ComputeBsplinesDegreeElevation1D(d1, ctrl_u, k1, t1, ictrl_u, ik1);
            ...
            ValuesContainerType2& ik3) // new knot vector
    {
        ...
        for (int i = 0; i < n1; ++i)
        {
            for (int j = 0; j < n2; ++j)
            {
                std::vector<DataType> ctrl_w(n3);
                std::vector<DataType> ictrl_w(n3); // n3 is just temporary size
                ...
                ComputeBsplinesDegreeElevation1D(d3, ctrl_w, k3, t3, ictrl_w, ik3);
