...
public:
    /// Pointer definition
    ISOGEOMETRIC_CLASS_POINTER_DEFINITION(HBSplinesRefinementUtility);
...
//                            pnew_bfs[i_func]->AddBoundary(BOUNDARY_FLAG(_BTOP_));
    ...
    */
    ...
    {
        typename WeightedFESpace<TDim>::Pointer pThisFESpace = Kratos::dynamic_pointer_cast<WeightedFESpace<TDim> >((*it)->pFESpace());
    ...
    for (std::size_t i = 0; i < pPatch->NumberOfInterfaces(); ++i)
    {
        ...
        // extract the hierarchical B-Splines space
        typename HBSplinesFESpace<TDim>::Pointer pNeighborFESpace = Kratos::dynamic_pointer_cast<HBSplinesFESpace<TDim> >(pNeighborPatch->pFESpace());
