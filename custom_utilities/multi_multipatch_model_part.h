...
public:
    /// Pointer definition
    ISOGEOMETRIC_CLASS_POINTER_DEFINITION(MultiMultiPatchModelPart);
    ...
    /// Default constructor
    MultiMultiPatchModelPart()
    : mIsModelPartReady(false), mModelPartName("MultiMultiPatch")
    {
    ...
    void CreateNodes()
    {
        ...
        for (std::size_t ip = 0; ip < mpMultiPatches.size(); ++ip)
        {
            ...
            for (std::size_t i = 0; i < mpMultiPatches[ip]->EquationSystemSize(); ++i)
            {
                ...
                NodeType::Pointer pNewNode = this->GetModelPart().CreateNewNode(CONVERT_INDEX_IGA_TO_KRATOS(node_counter), point.X(), point.Y(), point.Z());
                ++node_counter;
            }
        }
            ...
            const std::string& condition_name, const std::size_t& starting_id, Properties::Pointer pProperties)
    {
        ...
        ModelPart::ConditionsContainerType pNewConditions = MultiPatchModelPart<TDim>::template CreateEntitiesFromFESpace<Condition, FESpace<TDim-1>, ControlGrid<ControlPointType>, ModelPart::NodesContainerType>(pBoundaryPatch->pFESpace(), rControlPointGridFunction.pControlGrid(), this->GetModelPart().Nodes(), condition_name, starting_id, pProperties);
    ...
    void SynchronizeForward(const TVariableType& rVariable)
    {
        ...
        for (std::size_t ip = 0; ip < mpMultiPatches.size(); ++ip)
        {
            ...
            for (std::size_t i = 0; i < mpMultiPatches[ip]->EquationSystemSize(); ++i)
            {
                ...
                NodeType::Pointer pNode = this->GetModelPart().pGetNode(CONVERT_INDEX_IGA_TO_KRATOS(cnt++));
        ...
        for (typename cell_container_t::iterator it_dummy = pCellManagers[0]->begin(); it_dummy != pCellManagers[0]->end(); ++it_dummy)
        {
            ...
            for (std::size_t ip = 0; ip < pFESpaces.size(); ++ip)
            {
                ...
                typename IsogeometricGeometryType::Pointer p_temp_geometry
                    = Kratos::dynamic_pointer_cast<IsogeometricGeometryType>(r_clone_element.GetGeometry().Create(temp_element_nodes));
