...
//        KRATOS_WATCH(*pNewControlPoints)
        ...
        {
            typename BSplinesPatchInterface<TDim>::Pointer pInterface = Kratos::dynamic_pointer_cast<BSplinesPatchInterface<TDim> >(*it);
            if (pInterface == NULL)
...
void MultiPatchRefinementUtility::DegreeElevate(typename Patch<TDim>::Pointer& pPatch, std::map<std::size_t, std::vector<int> >& refined_patches, const std::vector<std::size_t>& order_increment)
{
    ...
    if (to_refine)
    {
        ...
        typename StructuredControlGrid<TDim, ControlPoint<double> >::Pointer pControlPoints
            = Kratos::dynamic_pointer_cast<StructuredControlGrid<TDim, ControlPoint<double> > >(pPatch->pControlPointGridFunction()->pControlGrid());
                ...
                it != DoubleGridFunctions_.end(); ++it)
        {
            ...
            typename StructuredControlGrid<TDim, double>::Pointer pDoubleControlGrid = Kratos::dynamic_pointer_cast<StructuredControlGrid<TDim, double> >((*it)->pControlGrid());
                ...
                it != Array1DGridFunctions_.end(); ++it)
        {
            ...
            typename StructuredControlGrid<TDim, array_1d<double, 3> >::Pointer pArray1DControlGrid = Kratos::dynamic_pointer_cast<StructuredControlGrid<TDim, array_1d<double, 3> > >((*it)->pControlGrid());
                ...
                it != VectorGridFunctions_.end(); ++it)
        {
            ...
            typename StructuredControlGrid<TDim, Vector>::Pointer pVectorControlGrid = Kratos::dynamic_pointer_cast<StructuredControlGrid<TDim, Vector> >((*it)->pControlGrid());
