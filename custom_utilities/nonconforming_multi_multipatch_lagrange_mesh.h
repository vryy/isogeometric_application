...
#if !defined(KRATOS_ISOGEOMETRIC_APPLICATION_NONCONFORMING_MULTI_MULTIPATCH_LAGRANGE_MESH_H_INCLUDED)
#define  KRATOS_ISOGEOMETRIC_APPLICATION_NONCONFORMING_MULTI_MULTIPATCH_LAGRANGE_MESH_H_INCLUDED
...
/**
 * Construct the standard FEM mesh based on Lagrange basis functions from isogeometric multipatches.
 * This support exporting the values from multi Multipatch, which is useful for post-processing of coupled simulation.
 ...
 * At the end, the resulting model_part will have nodal values interpolated from patch of different multipatch.
 ...
 */
template<int TDim>
class NonConformingMultiMultipatchLagrangeMesh : public IsogeometricEcho
{
public:
    /// Pointer definition
    KRATOS_CLASS_POINTER_DEFINITION(NonConformingMultiMultipatchLagrangeMesh);
    ...
    /// Default constructor
    NonConformingMultiMultipatchLagrangeMesh() : mLastNodeId(0), mLastElemId(0)
    {}

    /// Destructor
    virtual ~NonConformingMultiMultipatchLagrangeMesh() {}

    /// Add the multipatch to the internal list. The first multipatch will be used to construct the FEM mesh
    ...
    {
        if (mpMultiPatches.size() == 0)
            return;

        typedef typename MultiPatchType::patch_iterator patch_iterator;
        for (patch_iterator it = mpMultiPatches[0]->begin(); it != mpMultiPatches[0]->end(); ++it)
        {
    ...
    void SetDivision(IndexType patch_id, int dim, IndexType num_division)
    {
        ...
        if (mpMultiPatches[0]->Patches().find(patch_id) == mpMultiPatches[0]->end())
        {
    ...
    void WriteModelPart(ModelPart& r_model_part) const
    {
        if (mpMultiPatches.size() == 0)
        {
            if (GetEchoLevel() > 0)
            {
                std::cout << "NonConformingMultiMultipatchLagrangeMesh::" << __FUNCTION__ << " - the multipatch list is empty. ModelPart stays the same." << std::endl;
            }
            return;
        }
        ...
        if (GetEchoLevel() > 0)
        {
            std::cout << "NonConformingMultiMultipatchLagrangeMesh::" << __FUNCTION__ << " - started" << std::endl;
        }
        ...
        for (auto it = mpMultiPatches[0]->ptr_begin(); it != mpMultiPatches[0]->ptr_end(); ++it)
        {
            if (!(*it)->Is(ACTIVE))
            {
                continue;
            ...
            std::vector<typename PatchType::Pointer> pPatches;
            pPatches.push_back(*it);
            for (std::size_t i = 1; i < mpMultiPatches.size(); ++i)
                pPatches.push_back(mpMultiPatches[i]->pGetPatch((*it)->Id()));

            this->AddElements(r_model_part, pPatches, rCloneElement, NodeCounter, ElementCounter, pNewProperties);
        }
        ...
        {
            std::vector<typename PatchType::Pointer> pPatches;
            for (std::size_t i = 0; i < mpMultiPatches.size(); ++i)
                pPatches.push_back(mpMultiPatches[i]->pGetPatch(mConditionMap[i].first));
            ...
            this->AddConditions(r_model_part, pPatches, side, rCloneCondition, NodeCounter, ConditionCounter, pProperties);
        }

        if (GetEchoLevel() > 0)
        {
            std::cout << "NonConformingMultiMultipatchLagrangeMesh::" << __FUNCTION__ << " - completed" << std::endl;
        }
    }

    /// create the elements out from the patch and add to the model_part
    ElementsContainerType AddElements(ModelPart& r_model_part, std::vector<typename PatchType::Pointer> pPatches, Element const& rCloneElement,
            std::size_t& starting_node_id, std::size_t& starting_elem_id, Properties::Pointer pProperties) const
    {
        ...
        {
            IsogeometricPostUtility::CreateNodeAndTransferValuesFromMultiplePatches(points_and_connectivities.first[i], pPatches, r_model_part, NodeCounter++);
        }
    ...
    ConditionsContainerType AddConditions(ModelPart& r_model_part, std::vector<typename PatchType::Pointer> pPatches, const BoundarySide side,
    ...
    {
        if (pPatches.size() < 1)
            return ConditionsContainerType();

        // construct the boundary patches
        std::vector<typename BoundaryPatchType::Pointer> pBoundaryPatches;

        for (std::size_t i = 0; i < pPatches.size(); ++i)
            pBoundaryPatches.push_back(pPatches[i]->ConstructBoundaryPatch(side));

        // get the sampling numbers
        std::size_t patch_id = pPatches[0]->Id();
        auto it_num = mNumDivision.find(patch_id);
        if (it_num == mNumDivision.end())
            KRATOS_ERROR << "NumDivision is not set for patch " << patch_id;
        ...
        // create conditions
        return this->AddConditions(r_model_part, pBoundaryPatches, div, rCloneCondition, starting_node_id, starting_cond_id, pProperties);
    }

    /// create the conditions out from the slice patch and add to the model_part
    ConditionsContainerType AddConditions(ModelPart& r_model_part, std::vector<typename PatchType::Pointer> pPatches, const int idir, const double xi,
            Condition const& rCloneCondition, std::size_t& starting_node_id, std::size_t& starting_cond_id, Properties::Pointer pProperties) const
    {
        if (pPatches.size() < 1)
            return ConditionsContainerType();

        // extract the slice patches
        std::vector<typename BoundaryPatchType::Pointer> pSlicedPatches;

        for (std::size_t i = 0; i < pPatches.size(); ++i)
            pSlicedPatches.push_back(pPatches[i]->ConstructSlicedPatch(idir, xi));
        ...
        // create conditions
        return this->AddConditions(r_model_part, pSlicedPatches, div, rCloneCondition, starting_node_id, starting_cond_id, pProperties);
    }

    /// create the conditions out from the boudnary patch and add to the model_part
    ConditionsContainerType AddConditions(ModelPart& r_model_part, std::vector<typename BoundaryPatchType::Pointer> pBoundaryPatches,
            ...
            std::size_t& starting_node_id, std::size_t& starting_cond_id, Properties::Pointer pProperties) const
    {
        ...
        for (std::size_t i = 0; i < points_and_connectivities.first.size(); ++i)
        {
            IsogeometricPostUtility::CreateNodeAndTransferValuesFromMultiplePatches(points_and_connectivities.first[i], pBoundaryPatches, r_model_part, NodeCounter++);
        }
    ...
    {
        rOStream << "NonConformingMultiMultipatchLagrangeMesh<" << TDim << ">";
    }
...
template<int TDim>
inline std::ostream& operator <<(std::ostream& rOStream, const NonConformingMultiMultipatchLagrangeMesh<TDim>& rThis)
{
...
#endif // KRATOS_ISOGEOMETRIC_APPLICATION_NONCONFORMING_MULTI_MULTIPATCH_LAGRANGE_MESH_H_INCLUDED defined
