...
public:
    ...
    BendingStripNURBSPatch(std::size_t Id,
                           ...
                           int Order)
    : PatchType(Id), BaseType(pPatch1, side1, pPatch2, side2), mNormalOrder(Order)
    {
    ...
    : PatchType(Id), BaseType(pPatch1, side1, pPatch2, side2), mNormalOrder(Orders[TDim-1])
    {
        // check if the order is even
        if (this->NormalOrder() % 2 != 0)
            KRATOS_ERROR << "The strip order is not even, but " << this->NormalOrder();
...
public:
    ...
    BendingStripNURBSPatch(std::size_t Id,
                           ...
                           typename PatchType::Pointer pPatch2, const BoundarySide& side2,
                           int Order)
    : PatchType(Id), BaseType(pPatch1, side1, pPatch2, side2)
    {}
};
